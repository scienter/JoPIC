...
void MPI_TransferP_Period_X(Domain *D,Particle ***particle,int nSpecies);
void MPI_TransferP_Xplus(Domain *D,Particle ***particle,int nSpecies);
void MPI_TransferP_Xminus(Domain *D,Particle ***particle,int nSpecies);

void particleShareX(Domain D)
{
  MPI_TransferP_Xplus(&D,D.particle,D.nSpecies);
  MPI_TransferP_Xminus(&D,D.particle,D.nSpecies);
  MPI_TransferP_Xplus(&D,D.track,1);
  MPI_TransferP_Xminus(&D,D.track,1);
	if(D.Period==ON) {
    MPI_TransferP_Period_X(&D,D.particle,D.nSpecies);
    MPI_TransferP_Period_X(&D,D.track,1);
	} else ;
}
...
void MPI_TransferP_Period_X(Domain *D,Particle ***particle,int nSpecies)
{
  ...
  jstart=0;             jend=1;
  kstart=0;             kend=1;  
	if(D->dimension>1) {	jstart=D->jstart-1; jend=D->jend+1; nySub=D->nySub; } else ;
  ...
  // Right -> Left
	...
	if(D->L>1) {
		if(rank==D->L-1) 
			MPI_Send(&numP,1, MPI_INT,myrank-(D->L-1)*D->M*D->N, myrank, MPI_COMM_WORLD);    
		else if(rank==0) 
			MPI_Recv(&numP,1,MPI_INT,myrank+(D->L-1)*D->M*D->N,myrank+(D->L-1)*D->M*D->N,MPI_COMM_WORLD,&status);      
	   ...
	   for(j=jstart; j<jend; j++)
			for(k=kstart; k<kend; k++)
      		for(s=0; s<nSpecies; s++)  {
					p=particle[iend][j][k].head[s]->pt;
            	while(p)   {
	            	data[n*sendData+0]=p->x;     
   	           	data[n*sendData+1]=p->oldX-nxSub;  
         	     	...
         	     	data[n*sendData+17]=p->oldX2-nxSub;  
		...
		if(rank==D->L-1)  
		{
      	...
      	MPI_Send(data,totalData, MPI_DOUBLE,myrank-(D->L-1)*D->M*D->N,myrank, MPI_COMM_WORLD); 
		}
		else if(rank==0)   
		{    
      	MPI_Recv(data,totalData, MPI_DOUBLE,myrank+(D->L-1)*D->M*D->N,myrank+(D->L-1)*D->M*D->N,MPI_COMM_WORLD,&status);      
      	for(n=0; n<numP; n++)  {
        		j=(int)(data[n*sendData+2]);
       		k=(int)(data[n*sendData+4]);
        		s=(int)(data[n*sendData+10]);
	...
	if(D->L==1) 
	{
	   ...
	   for(i=0; i<istart; i++)
		...
		for(n=0; n<numP; n++)  {
        	...
        	New->oldX=data[n*sendData+1]+nxSub;     
        	New->y=data[n*sendData+2]-j;     
        	New->oldY=data[n*sendData+3];     
        	New->z=data[n*sendData+4]-k;     
        	New->oldZ=data[n*sendData+5];     
        	...
        	New->p2Old1=data[n*sendData+15];
        	New->p3Old1=data[n*sendData+16];
        	New->oldX2=data[n*sendData+17]+nxSub;
        	New->oldY2=data[n*sendData+18];
        	New->oldZ2=data[n*sendData+19];
     	}
	} else {
      ...
      for(k=kstart; k<kend; k++)
        for(s=0; s<nSpecies; s++)
        {
          ...
          while(p)   
          {
            ...
            upP[n*sendData+17]=p->oldX2-nxSub;  
            upP[n*sendData+18]=p->oldY2;  
   ...
   rank=myrank/(D->M*D->N);
    ...
    {    
      ...
      for(n=0; n<numP; n++)
      {
        ...
        New->oldX2=btP[n*sendData+17]+nxSub;
        New->oldY2=btP[n*sendData+18];
