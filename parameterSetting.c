...
void parameterSetting(Domain *D,External *Ext, char *input)
{
   ...
   if(D->M*D->N!=nTasks)  {
     printf("L=%d, M=%d, N=%d, check the values of M and N.\n",D->L,D->M,D->N);
     fail=1;
   }
   ...
   if(FindParameters("Domain",1,"field_type",input,str)) 
     D->fieldType=whatFieldType(str);
      ...
      printf("in [Domain], field_type=?  (Split or Yee)\n");
      fail=1;
   }
   //Current Type
   ...
   if(FindParameters("Domain",1,"field_save",input,str)) 
     ...
     D->fieldSave=ON;
   if(FindParameters("Domain",1,"raman_save",input,str)) 
   ...
   //pml
   if(FindParameters("Domain",1,"pml",input,str)) 
     D->pmlOn=whatONOFF(str);
   else  
     D->pmlOn=0;
   if(FindParameters("Domain",1,"pml_cells",input,str))
     D->pmlCell=atoi(str);
   else  {
     printf("In [Domain], pml_cells=? [ea]\n");
     fail=1;
   }
   if(FindParameters("Domain",1,"pml_r",input,str))
     D->pmlr=atof(str);
   else  {
     printf("In [Domain], pml_r=? (retarding length)\n");
     fail=1;
   }
   if(FindParameters("Domain",1,"pml_d",input,str))
     D->pmld=atof(str);
   else  {
     printf("In [Domain], pml_d=? (damping length)\n");
     fail=1;
   }
   ...
   if(D->dimension>1)
   {
     ...
     if(D->fieldType==Yee)
       D->dt=tmp/sqrt(1.0/D->dx/D->dx+1.0/D->dy/D->dy);
     else	;
   }
   else	;
   if(D->dimension>2)
   {
     ...
     if(D->fieldType==Yee)
       D->dt=tmp/sqrt(1.0/D->dx/D->dx+1.0/D->dy/D->dy+1.0/D->dz/D->dz);
     else	;
   }
   else	;   

   D->minYDomain=D->minZDomain=0;
...
//      LL->superP=LL->density*D->lambda*D->dx*D->lambda*D->dy*D->lambda*D->dz/LL->numberInCell;
      ...
      case Polygon :
        ...
        {	
          ...
          if(FindParameters("Plasma",rank,"poly_coef_YZ",input,str))  
            LL->polyCoefYZ=atof(str)/D->lambda/sqrt(D->dy*D->dy+D->dz*D->dz);
          else   LL->polyCoefYZ=0.0;	
        }
        else 	;
        break;
