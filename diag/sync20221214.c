...
void main(int argc, char *argv[])
{
   ...
   if(argc < 11)   { 
		if(myrank==0) {
	      printf("sync mode correctionONOFF scrAngle scrD scrN minPhotonE maxPhotonE numPhoton screenX0 numericONOFF jumpT\n");
	...
	for(idx=0; idx<D->scrN; idx++)  
	{
		...
		for(ii=startI; ii<numW; ii++) 
		{
            ...
            for(i=0; i<3; i++) {
				   ...
				   nextR[i]=trackParticle[(nn+jump)*6+i];
					prevR[i]=trackParticle[(nn-jump)*6+i];
				...
				for(i=0; i<3; i++) {
				   u1[i]=(nextU[i]-prevU[i])*gamma/dt;
					...
					r1[i]=gamma*0.5/dt*(nextR[i]-prevR[i]); 
					r2[i]=gamma*gamma*0.5/dt/dt*(nextR[i]+prevR[i]-2*r0[i]);
				}
				...
				nDotV=0.0;     for(i=0; i<3; i++) { nDotV+=n[i]*u0[i]; }
				...
				if(D->numeric==OFF) {
				   ...
				   if(chi1!=0.0 && chi2!=0.0) {			
				      if(test<1e-3) {    // Taylor expansion calculation
							...
							Th_p=(chi1+absChi2*dt*invGam)*tmp;
							Th_m=(chi1-absChi2*dt*invGam)*tmp;
							...
							Phi_p=0.25*dt*dt*absChi2*invGam*invGam+sign*0.5*dt*chi1*invGam;
							Phi_m=0.25*dt*dt*absChi2*invGam*invGam-sign*0.5*dt*chi1*invGam;
							for(i=0; i<3; i++) {
								Ireal[i]=0.25*invGam/absChi2*(Psi_p[i]*(fresC_p-fresC_m)+Psi_m[i]*(fresS_p-fresS_m)+2*u1[i]*(sin(Phi_p)-sin(Phi_m)));
								Iimag[i]=0.25*invGam/absChi2*(Psi_p[i]*(fresS_p-fresS_m)-Psi_m[i]*(fresC_p-fresC_m)-2*u1[i]*(cos(Phi_p)-cos(Phi_m)))*sign;
							}
					...
					phase1=w*t;
					for(i=0; i<3; i++) phase1-=w/c*n[i]*r0[i];
					cosP1=cos(phase1);
					sinP1=sin(phase1);
	
					for(i=0; i<3; i++) {
						spectra1[ii][idx][i]+=w*(n[i]*nDotV-u0[i])*sinP1*weight*invGam*dt;
						spectra2[ii][idx][i]-=w*(n[i]*nDotV-u0[i])*cosP1*weight*invGam*dt;
					}
				}
