...
//						printf("nn=%d, chi1=%g, chi2=%g\n",nn,chi1,chi2);
					...
					for(i=0; i<3; i++) {
						spectraR[ii][idx][i]+=w*(n[i]*nDotV-b0[i])*sinP1*weight*dt;
						spectraI[ii][idx][i]-=w*(n[i]*nDotV-b0[i])*cosP1*weight*dt;
					}
				}
...
//		   		invR=1.0/sqrt(R2);	for(i=0; i<3; i++)	n[i]=delR[i]*invR;
					...
					for(i=0; i<3; i++) {
						spectraR[ii][idx][i]+=amp[i]*cosP1;
						spectraI[ii][idx][i]+=amp[i]*sinP1;
					}
...
//		   		invR=1.0/sqrt(R2);	for(i=0; i<3; i++)	n[i]=delR[i]*invR;
					...
					for(i=0; i<3; i++) {
						spectraR[ii][idx][i]-=amp[i]*cosP2;
						spectraI[ii][idx][i]-=amp[i]*sinP2;
					}
				} else ;
		...
		if(myrank==0 && nn%100==0) { 
			printf("step %d/%d is done for %d/%d.\n",nn,maxStep,index,end); 
		} else ;
