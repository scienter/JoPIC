...
void MPI_TransferJ_Yplus(Domain *D,double ***f1,double ***f2,double ***f3,int nx,int nz)
{
    ...
    for(j=0; j<3; j++)
      for(k=0; k<nz; k++)
      {
        for(i=ibegin; i<ibottom; i++)
          D->YplusJ[start+i]=f1[i][jend+j][k];
        start+=ibottom;
        for(i=ibegin; i<ibottom; i++)
          D->YplusJ[start+i]=f2[i][jend+j][k];
        start+=ibottom;
        for(i=ibegin; i<ibottom; i++)
          D->YplusJ[start+i]=f3[i][jend+j][k];
        start+=ibottom;
      }
    ...
    if(rank%2==1)
    {
       ...
       for(j=0; j<3; j++)
         ...
         {
           for(i=ibegin; i<ibottom; i++)
             f1[i][jstart+j][k]+=D->YplusJ[i+start];
           start+=ibottom;
           for(i=ibegin; i<ibottom; i++)
             f2[i][jstart+j][k]+=D->YplusJ[i+start];
           start+=ibottom;
           for(i=ibegin; i<ibottom; i++)
             f3[i][jstart+j][k]+=D->YplusJ[i+start];
           start+=ibottom;
         }
    }
...
void MPI_TransferJ_Yminus(Domain *D,double ***f1,double ***f2,double ***f3,int nx,int nz)
{
    ...
    for(j=1; j<3; j++)
      ...
      {
        for(i=ibegin; i<ibottom; i++)
          D->YminusJ[start+i]=f1[i][jstart-j][k];
        start+=ibottom;
        for(i=ibegin; i<ibottom; i++)
          D->YminusJ[start+i]=f2[i][jstart-j][k];
        start+=ibottom;
        for(i=ibegin; i<ibottom; i++)
          D->YminusJ[start+i]=f3[i][jstart-j][k];
        start+=ibottom;
      }
    if(rank%2==1 && rank!=D->M-1)
    {
       ...
       for(j=1; j<3; j++)
         ...
         {
           for(i=ibegin; i<ibottom; i++)
             f1[i][jend-j][k]+=D->YminusJ[i+start];
           start+=ibottom;
           for(i=ibegin; i<ibottom; i++)
             f2[i][jend-j][k]+=D->YminusJ[i+start];
           start+=ibottom;
           for(i=ibegin; i<ibottom; i++)
             f3[i][jend-j][k]+=D->YminusJ[i+start];
           start+=ibottom;
         }
    }
...
//    numberData=1*3*nx*nz; 
    ...
    if(rank%2==0 && rank!=D->M-1)
    {
      MPI_Recv(D->minusYC,D->numMinusYC, MPI_DOUBLE, myrank+1, myrank+1, MPI_COMM_WORLD,&status);  
      start=0; 
      for(j=0; j<1; j++)	// 3 layers in y direction
        for(k=0; k<nz; k++)
        {
       ...
       MPI_Send(D->minusYC,D->numMinusYC, MPI_DOUBLE, myrank-1, myrank, MPI_COMM_WORLD);             
    MPI_Barrier(MPI_COMM_WORLD);
...
//    numberData=share*6*nx*nz; 
    ...
    if(rank%2==0 && rank!=D->M-1)
    {
      MPI_Recv(D->minusY,D->numMinusY, MPI_DOUBLE, myrank+1, myrank+1, MPI_COMM_WORLD,&status);  
      start=0; 
      for(j=0; j<share; j++)	// 3 layers in y direction
        for(k=0; k<nz; k++)
        {
       ...
       MPI_Send(D->minusY,D->numMinusY, MPI_DOUBLE, myrank-1, myrank, MPI_COMM_WORLD);             
    MPI_Barrier(MPI_COMM_WORLD);
...
//    numberData=1*3*nx*nz; 
    ...
    if(rank%2==1)
    {
       MPI_Recv(D->plusYC,D->numPlusYC, MPI_DOUBLE, myrank-1, myrank-1, MPI_COMM_WORLD,&status);  
       start=0;
       for(j=1; j<2; j++)
         for(k=0; k<nz; k++)
         {
       ...
       MPI_Send(D->plusYC,D->numPlusYC, MPI_DOUBLE, myrank+1, myrank, MPI_COMM_WORLD);             
    MPI_Barrier(MPI_COMM_WORLD);
...
//    numberData=(share-1)*6*nx*nz; 
    ...
    if(rank%2==1)
    {
       MPI_Recv(D->plusY,D->numPlusY, MPI_DOUBLE, myrank-1, myrank-1, MPI_COMM_WORLD,&status);  
       start=0;
       for(j=1; j<share; j++)
         for(k=0; k<nz; k++)
         {
       ...
       MPI_Send(D->plusY,D->numPlusY, MPI_DOUBLE, myrank+1, myrank, MPI_COMM_WORLD);             
    MPI_Barrier(MPI_COMM_WORLD);
