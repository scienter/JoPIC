...
{
  int s;
  float minPx[nSpecies];
  void saveParticle_HDF(Domain D,int iteration,int s,float minPx);
  LoadList *LL;
...
void saveParticle_HDF(Domain D,int iteration,int s,float minPx)
{
    ...
    float dx,dy,dz,lambda,tmpFloat;
    char name[100];
    float *saveFloat;
...
//    H5Pset_fclose_degree(plist_id,H5F_CLOSE_SEMI);
    ...
    case 2:
      ...
      for(i=0; i<nTasks; i++)
        totalCnt+=recv[i];
      saveFloat = (float *)malloc(cnt*sizeof(float ));      
      ...
      for(i=istart; i<iend; i++)
        for(j=jstart; j<jend; j++)
        {
          ...
          while(p)
          {
            if(p->p1>=minPx)
            {
              tmpFloat=((i-istart+minXSub)+p->x)*dx*lambda;
              saveFloat[index]=tmpFloat;
              index++;
            }
            else 	;
            p=p->next;
      ...
      saveParticleComp_Float(saveFloat,name,"x",totalCnt,cnt,start);
      index=0;
      for(i=istart; i<iend; i++)
        for(j=jstart; j<jend; j++)
        {
          ...
          while(p)
          {
            if(p->p1>=minPx)
            {
              tmpFloat=((j-jstart+minYSub)+p->y)*dy*lambda;
              saveFloat[index]=tmpFloat;
              index++;
            }
            else	;
      ...
      saveParticleComp_Float(saveFloat,name,"y",totalCnt,cnt,start);
      index=0;
      for(i=istart; i<iend; i++)
        for(j=jstart; j<jend; j++)
        {
      ...
      saveParticleComp_Float(saveFloat,name,"px",totalCnt,cnt,start);
      index=0;
      for(i=istart; i<iend; i++)
        for(j=jstart; j<jend; j++)
        {
      ...
      saveParticleComp_Float(saveFloat,name,"py",totalCnt,cnt,start);
      index=0;
      for(i=istart; i<iend; i++)
        for(j=jstart; j<jend; j++)
        {
      ...
      saveParticleComp_Float(saveFloat,name,"pz",totalCnt,cnt,start);
      index=0;
      for(i=istart; i<iend; i++)
        for(j=jstart; j<jend; j++)
        {
    ...
    case 3:
        ...
        for(j=jstart; j<jend; j++)
          for(k=kstart; k<kend; k++)
          {
            ...
            while(p)
            {
              if(p->p1>=minPx)
              {
                tmpFloat=((k-kstart+minZSub)+p->z)*dz*lambda;
                saveFloat[index]=tmpFloat;
                index++;
              }
              else	;
      ...
      saveParticleComp_Float(saveFloat,name,"z",totalCnt,cnt,start);
      index=0;
      for(i=istart; i<iend; i++)
        for(j=jstart; j<jend; j++)
          for(k=kstart; k<kend; k++)
...
void saveParticleComp_Float(float *data,char *fileName,char *dataName,int totalCnt,int cnt,int offSet)
{
  int i,j,k;
  int myrank, nTasks;
  MPI_Comm_rank(MPI_COMM_WORLD, &myrank);
...
//    MPI_Barrier(MPI_COMM_WORLD);
  ...
  status = H5Dwrite(dset_id, H5T_NATIVE_FLOAT,memspace,subfilespace,plist_id,data);
  H5Pclose(plist_id);
