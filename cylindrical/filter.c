...
void filter_center(Domain *D,double ***dataR,double ***dataI,int iteration)
{
  ...
  m=0;
    for(j=0; j<nySub; j++) 
      for(i=istart; i<iend; i++) {
        ...
        sumI=0.0;
        for(ii=0; ii<3; ii++) {
          sumR+=wz[ii]*dataR[m][i-1+ii][j];
          sumI+=wz[ii]*dataI[m][i-1+ii][j];
		  }
        valR[m][i][j]=sumR;
        valI[m][i][j]=sumI;
      }
  ...
  if(D->L>1)  {
    MPI_filterCenter_Xplus(D,valR,valI,nySub,3);
    MPI_filterCenter_Xminus(D,valR,valI,nySub,3);
  } else ;

  m=0; j=jstart;
    for(i=istart; i<iend; i++) {
      dataR[m][i][j]=valR[m][i][j];
      dataI[m][i][j]=valI[m][i][j];
    }
