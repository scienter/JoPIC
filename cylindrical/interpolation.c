...
void interpolation_Split_1st(Domain *D,External *Ext,int iteration)
{
   int ii,jj,i,j,m,i1,j1,istart,iend,jstart,jend,s,rank,numMode,tmpInt;
    ...
    for(j=jstart+1; j<jend; j++) 
      for(s=0; s<D->nSpecies; s++)
      {
        ...
        while(p) {
          ...
          tmpInt=(int)(r+0.5);
          WR[0]=(2*(j-jstart)+r-0.5+tmpInt)*(0.5+tmpInt-r)*0.5*invR; WR[1]=1.0-WR[0];
          wz[1]=z;             wz[0]=1.0-wz[1];
          wr[0]=(j-jstart+R)*(1-r)*0.5*invR;  wr[1]=1-wr[0];
    ...
    for(i=istart; i<iend; i++)
      for(s=0; s<D->nSpecies; s++)
      {
        ...
        while(p) {
          ...
          if(r<0.5) {
            if(r<0.25) WR[0]=(8*r*r-4*r+1)/(16*r*r+1.0);
            else       WR[0]=0.5-r;
            WR[1]=1-WR[0];

            //WR[0]=(0.25*0.25+(r-0.5)*(r-0.5))/(2*r*r+0.5); WR[1]=1-WR[0];
            wr[0]=(r*r-r+0.5)/(2*r*r+0.5);                 wr[1]=1-wr[0];

            Ez=Bz=Pr1=Pl1=Sr1=Sl1=0.0;
            m=0;
			     for(ii=0; ii<2; ii++) 
			       for(jj=1; jj<2; jj++) {
				  	   Pr1+=WZ[ii]*WR[jj]*D->PrR[m][i1-1+ii][j1-1+jj];
				  	   Pl1+=WZ[ii]*WR[jj]*D->PlR[m][i1-1+ii][j1-1+jj];
				  	   Sr1+=WZ[ii]*WR[jj]*D->SrR[m][i1-1+ii][j1-1+jj];
				  	   Sl1+=WZ[ii]*WR[jj]*D->SlR[m][i1-1+ii][j1-1+jj];
				    }
            m=1;
  			     for(ii=0; ii<2; ii++) 
			       for(jj=1; jj<2; jj++) {
            ...
            for(m=2; m<numMode; m++) 
  			     for(ii=0; ii<2; ii++)  
  			       for(jj=1; jj<2; jj++) { 
  			  	      Pr1+=WZ[ii]*WR[jj]*D->PrR[m][i1-1+ii][j1-1+jj]*coss[m];
  			  	      Pr1-=WZ[ii]*WR[jj]*D->PrI[m][i1-1+ii][j1-1+jj]*sins[m];
				  	   Pl1+=WZ[ii]*WR[jj]*D->PlR[m][i1-1+ii][j1-1+jj]*coss[m];
				  	   Pl1-=WZ[ii]*WR[jj]*D->PlI[m][i1-1+ii][j1-1+jj]*sins[m];
				  	   Sr1+=WZ[ii]*WR[jj]*D->SrR[m][i1-1+ii][j1-1+jj]*coss[m];
				  	   Sr1-=WZ[ii]*WR[jj]*D->SrI[m][i1-1+ii][j1-1+jj]*sins[m];
				  	   Sl1+=WZ[ii]*WR[jj]*D->SlR[m][i1-1+ii][j1-1+jj]*coss[m];
				  	   Sl1-=WZ[ii]*WR[jj]*D->SlI[m][i1-1+ii][j1-1+jj]*sins[m];
                }
            Er=0.5*(Pr1+Pl1);
            Ep=0.5*(Sr1+Sl1);
            Br=0.5*(Sl1-Sr1);
            ...
            WR[0]=(0.5+r)*(1.5-r)/(2*r);       WR[1]=1-WR[0];
            wr[0]=(r+1.0)*0.5;                 wr[1]=1-wr[0];
...
void interpolation_Yee_1st(Domain *D,External *Ext)
{
   int i,j,m,i1,j1,ii,jj,istart,iend,jstart,jend,s,numMode,tmpInt;
       ...
       for(s=0; s<D->nSpecies; s++)
       {
         ...
         while(p) {
           ...
           if(r<0.5) {
             ...
             for(ii=0; ii<2; ii++) 
               for(jj=1; jj<2; jj++) {
                 Bz+=wz[ii]*D->BzNowR[m][i+ii][j1-1+jj];
                 Bp+=WZ[ii]*WR[jj]*D->BpNowR[m][i1-1+ii][j1-1+jj];
                 Er+=wz[ii]*WR[jj]*D->ErR[m][i+ii][j1-1+jj];
               }
             m=1;
             for(ii=0; ii<2; ii++) 
               for(jj=1; jj<2; jj++) {
                 Bz+=wz[ii]*WR[jj]*D->BzNowR[m][i+ii][j1-1+jj]*coss[m]
                    -wz[ii]*WR[jj]*D->BzNowI[m][i+ii][j1-1+jj]*sins[m];
                 Bp+=WZ[ii]*D->BpNowR[m][i1-1+ii][j1-1+jj]*coss[m]
                    -WZ[ii]*D->BpNowI[m][i1-1+ii][j1-1+jj]*sins[m];
             ...
             for(m=2; m<numMode; m++) 
               for(ii=0; ii<2; ii++) 
                 for(jj=1; jj<2; jj++) {
                   Bz+=wz[ii]*WR[1]*D->BzNowR[m][i+ii][j1-1+jj]*coss[m]
                      -wz[ii]*WR[1]*D->BzNowI[m][i+ii][j1-1+jj]*sins[m];
                   Bp+=WZ[ii]*WR[1]*D->BpNowR[m][i1-1+ii][j1-1+jj]*coss[m]
                      -WZ[ii]*WR[1]*D->BpNowI[m][i1-1+ii][j1-1+jj]*sins[m];
                   Er+=wz[ii]*WR[1]*D->ErR[m][i+ii][j1-1+jj]*coss[m]
                      -wz[ii]*WR[1]*D->ErI[m][i+ii][j1-1+jj]*sins[m];
                 }
             ...
             for(ii=0; ii<2; ii++)
               for(jj=0; jj<2; jj++) {
                 Bz+=wz[ii]*WR[jj]*D->BzNowR[m][i+ii][j1-1+jj];
                 Bp+=WZ[ii]*WR[jj]*D->BpNowR[m][i1-1+ii][j1-1+jj];
                 Er+=wz[ii]*WR[jj]*D->ErR[m][i+ii][j1-1+jj];
               }
             for(m=1; m<numMode; m++) 
           ...
           //Ez, Ep, Br
           if (r<0.5) wr[0]=(r*r-r+0.5)/(2*r*r+0.5);
           else       wr[0]=(r+1.0)*0.5;
           wr[1]=1-wr[0];
           ...
           m=0;
             for(ii=0; ii<2; ii++) { 
               for(jj=1; jj<2; jj++) {
                 Br+=WZ[ii]*D->BrNowR[m][i1-1+ii][j+jj];
                 Ep+=wz[ii]*wr[jj]*D->EpR[m][i+ii][j+jj];
               }
               for(jj=0; jj<2; jj++) 
                 Ez+=WZ[ii]*wr[jj]*D->EzR[m][i1-1+ii][j+jj];
             }
