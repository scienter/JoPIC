...
//printf("myrank=%d,offset[0]=%d,offset[1]=%d,nxSub=%d,nySub=%d\n",myrank,offset[0],offset[1],nxSub,nySub);
    ...
    if(D->fieldType!=Split) {
      ...
      restoreFieldComp(D->EzNowR,fieldFile,"/EzNowR",nx,ny,nxSub,nySub,istart,iend,jstart,jend,numMode,offset);
      restoreFieldComp(D->EzNowI,fieldFile,"/EzNowI",nx,ny,nxSub,nySub,istart,iend,jstart,jend,numMode,offset);
      restoreFieldComp(D->BzNowR,fieldFile,"/BzNowR",nx,ny,nxSub,nySub,istart,iend,jstart,jend,numMode,offset);
      restoreFieldComp(D->BzNowI,fieldFile,"/BzNowI",nx,ny,nxSub,nySub,istart,iend,jstart,jend,numMode,offset);
    }
...
//          while(flag==0 && rank<nTasks)  {
            if(minXSub[rank]<=z && z<maxXSub[rank] &&
              ...
              if(rank==myrank)  {
                ...
                p->x=x;
                p->y=y;
                p->oldZ=data[i*dataCnt+10];
                p->oldX=data[i*dataCnt+11];
                p->oldY=data[i*dataCnt+12];
        ...
        {          
          ...
          if(myrank!=core && core>=0 && core<nTasks)  {
            ...
            sendData[core][n*dataCnt+10]=data[i*dataCnt+10];
            sendData[core][n*dataCnt+11]=data[i*dataCnt+11];
            sendData[core][n*dataCnt+12]=data[i*dataCnt+12];
            coreCnt[core]+=1;
          }	else	;
        }
        ...
        for(i=0; i<nTasks; i++)
        {
          if(myrank==i)  {
            ...
            for(j=0; j<recvDataCnt[i]; j++)  {
              ...
              p->x=x;
              p->y=y;
              p->oldZ=recvData[i][j*dataCnt+10];
              p->oldX=recvData[i][j*dataCnt+11];
              p->oldY=recvData[i][j*dataCnt+12];
