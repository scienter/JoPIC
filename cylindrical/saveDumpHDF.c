...
void saveDumpParticleHDF(Domain *D,int iteration,char *fileName)
{
    int i,j,s,istart,iend,jstart,jend,dataCnt=13;
        ...
        for(i=istart; i<iend; i++)
          for(j=jstart; j<jend; j++) {
            ...
            while(p)    {
              ...
              data[index*dataCnt+10]=p->oldZ;
              data[index*dataCnt+11]=p->oldX;
              data[index*dataCnt+12]=p->oldY;
              index++;
              p=p->next;
            }
          }
...
//    sprintf(name,"dumpField%d.h5",iteration);
    ...
    if(D->fieldType!=Split) {
      ...
      saveFieldComp(D->SlI,fileName,"/SlI",nx,ny,nxSub,nySub,istart,iend,jstart,jend,numMode,offset);
    	saveFieldComp(D->EzNowR,fileName,"/EzNowR",nx,ny,nxSub,nySub,istart,iend,jstart,jend,numMode,offset);
	   saveFieldComp(D->EzNowI,fileName,"/EzNowI",nx,ny,nxSub,nySub,istart,iend,jstart,jend,numMode,offset);
   	saveFieldComp(D->BzNowR,fileName,"/BzNowR",nx,ny,nxSub,nySub,istart,iend,jstart,jend,numMode,offset);
	   saveFieldComp(D->BzNowI,fileName,"/BzNowI",nx,ny,nxSub,nySub,istart,iend,jstart,jend,numMode,offset);
    }
}
